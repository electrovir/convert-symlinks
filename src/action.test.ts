...
import {assertTestContext, describe, it, type UniversalTestContext} from '@augment-vir/test';
import {mkdir, readdir, readFile, rm, stat, writeFile} from 'node:fs/promises';
import {dirname, join, sep} from 'node:path';
import type {RequireExactlyOne} from 'type-fest';
import {convertAllSymlinks, ignoredPaths} from './action.js';

type FilesToGenerate = {
    [Path: string]: RequireExactlyOne<{
        symlink: string;
        contents: string;
    }>;
};
...
async function countAllSymlinks(dirPath: string): Promise<number> {
...
async function assertLinkEquality(testDirPath: string, filesToGenerate: FilesToGenerate) {
    await Promise.all(
        Object.entries(filesToGenerate).map(
            async ([
                filePath,
                options,
            ]) => {
...
async function populateTestFiles(testDirPath: string, filesToGenerate: FilesToGenerate) {
    const symlinkEntries: Record<
        /** Symlink file path. */
        string,
        /** Symlink target path. */
        string
     ...
     */
    await Promise.all(
        Object.entries(symlinkEntries).map(
            async ([
                symlinkPath,
                targetPath,
            ]) => {
...
async function testConvertAllSymlinks(
...
describe(convertAllSymlinks.name, () => {
    it('handles having no symlinks', async (context) => {
        assert.deepEquals(
            await testConvertAllSymlinks(context, {
                'not-link.txt': {
